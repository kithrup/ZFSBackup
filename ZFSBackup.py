...
class ZFSHelperCommand(ZFSHelper):
    ...
    def start(self):
        ...
        """
        if debug:
            print("***** in {}.start()".format(self.name), file=sys.stderr)
        self._proc = None
...
class ZFSBackupFilterBase(object):
    ...
    def start_backup(self, source):
        if debug:
            print("In {}.start_backup({}), stdout={}".format(self.name, source, self.stdout), file=sys.stderr)
        self.mode = "backup"
        self.stdin = source
        self.start()
        if debug:
...
#    def finish(self):
#        print("&&&&&&& in {}.finish()".format(self.name), file=sys.stderr)
#        self.wait()
    ...
    * tanks/usr/home -> backup/home means getting snapshots from backup/home/usr/home
    ...
    def _finish_filters(self, reason="backup"):
        ...
        for f in self.filters if reason == "backup" else reversed(self.filters):
            if debug:
                print("Calling {}.finish".format(f.name), file=sys.stderr)
            f.finish()
...
#        write_side_obj = os.fdopen(write_side, "wb", 1024*1024)
        ...
        # This runs the chunk reading in its own thread.
        # It's not using ZFSHelperThread, because that's not
        # set up to do the right thing.  (I would need to
        # subclass it and override some of the methods, which
